...
$dir = Join-Path $PSScriptRoot 'blog'
...
foreach ($f in $files) {
  ...
  # Root page links inside blog should use ../
  ...
  if ($c -ne $o) {
    try { Copy-Item -Path $f.FullName -Destination ($f.FullName + '.bak.bloglinks') -ErrorAction SilentlyContinue } catch {}
