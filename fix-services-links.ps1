...
$dir = Join-Path $PSScriptRoot 'services'
...
foreach ($f in $files) {
  ...
  # Use literal replacements to avoid regex escaping issues
  ...
  if ($c -ne $o) {
    try { Copy-Item -Path $f.FullName -Destination ($f.FullName + '.bak.servicelinks') -ErrorAction SilentlyContinue } catch {}
